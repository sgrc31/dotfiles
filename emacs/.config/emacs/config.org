...
;; I have a temporary key problem with the nongnu archive, gonna temporary disable the whole archive
...
* Key bindings
  ...
  (use-package which-key
    ...
    :config
    (which-key-mode)
    (setq which-key-idle-delay 0.3              ; Show faster (default is 1.0)
...
** TY language server
Use ty as the sole Python LSP server, explicitly blocking other registered servers.
#+BEGIN_SRC emacs-lisp
(use-package lsp-mode
